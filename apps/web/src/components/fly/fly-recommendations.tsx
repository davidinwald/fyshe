...
export function FlyRecommendations() {
  ...
  const { data: recommendations, isLoading, isError, error } = trpc.fly.recommend.useQuery(
    filters as Parameters<typeof trpc.fly.recommend.useQuery>[0],
    { enabled },
  );
  ...
  const results = (recommendations ?? []) as unknown as RecommendedPattern[];
